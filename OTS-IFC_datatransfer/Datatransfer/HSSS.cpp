...
int mem_flag[tgtbuf_num];//0可用，1不可用s
#define consume 1
int bulk_order = 0;
#define num_bulk_queue 1
...
double* speed_store;
int count_speed = 0;
int* memorypool_used_store;
int count_memorypool_used = 0;
...
static int write_header_record_to_file(ADQRecordHeader* phead, size_t headnum, void* pBuf, size_t datasize, int record)  //多少个点
{
	...
	if (time_diff > 999)
	{
		...
		maxbufferused[count_speed] = buffers_filled;
		memorypool_used_store[count_memorypool_used] = memory_pool_used;
		count_memorypool_used++;
		count_speed++;
...
static int write_perpare(void* pBuf, size_t datasize, int record)  //多少个点
{
	char filename[256] ;
	size_t bytes_written = 0;
	sprintf(filename, "%s_r%d.bin", "D://data/perpare", record);//写数据头
	fstream file(filename, ios::out | ios::binary);
	file.write(reinterpret_cast<char*>(pBuf), datasize * 2);
	file.close();
	return 0;
...
int write_memorypool(int* mmypool_add, int count)
{
	char filename[256] = "./data/memorypool.txt";
	FILE* fp = NULL;
	size_t written = 0;
...
int mmmproducer = 0;
int a = 0;
unsigned int mnn_thread[5];
unsigned int mnn_thread_w[5];
unsigned int effict_time[5];
void* consumeItem1(int ID)
{
	...
	tref[ID * 2 + 5] = 0;
	nodes tmp[consume];
	while ( popOver)// || q.size_approx()!=0
	{
		//timer_start(ID * 2 + 5);
		int counttt = q.wait_dequeue_bulk_timed(tmp, consume, std::chrono::milliseconds(0));//3
		//if (counttt == 1)printf("%d\n", timer_time_ms(ID * 2 + 5));
		//else printf("counnt=0, %d %d\n", ID, q.size_approx());
		for (int jjj = 0; jjj < counttt; jjj++)
		{
			timer_start(ID * 2 + 5);	//printf("%d\n",timer_time_ms(ID * 2 + 5));
			...
			//printf("%d\n",timer_time_ms(ID * 2 + 5));			
			effict_time[ID] += timer_time_ms(ID * 2 + 5);
			...
			//if(ID == 1 || ID == 2)
				//printf("consumeItem %d counttt %d file_number %d mnn_thread_w %d\n",ID, counttt, tmp[jjj].file_number, mnn_thread_w[ID]);
		//if (memory_pool_used < 10) {
		//std::this_thread::yield();
		//	std::this_thread::sleep_for(std::chrono::milliseconds(800));//1000msMMP
		//		//microseconds(300));//1000DRAM2000DRMA4000DRAM8000DRAM30000DRAM300000DRAm
		//	mmm++;
		//}		
...
void adq_transfer_test(void* adq_cu, int adq_num, int adq_type)
{
	...
	//thread t3(consumeItem1, 3);//thread t4(consumeItem1, 4);	thread t5(consumeItem1, 5);thread t6(consumeItem1, 6);
	//thread t7(consumeItem1, 7); thread t8(consumeItem1, 8);
	unsigned int asizem = sel_tr_buf_size * filefactor;
	for (int ii = 0; ii < tgtbuf_num; ii++)
	{
		mem_address[ii] = (short*)malloc(asizem);
	...
	memorypool_used_store = (int*)malloc(1024 * 4);
	//for (int ii = 0; ii < 100; ii++)
	//	write_perpare(mem_address[ii % tgtbuf_num], sizeof(short) * asizem / 2, ii);
	thread t(produceItem, adq_cu, adq_num, adq_type);

	//thread t5(consumeItem1, 3);// thread t6(consumeItem1, 4);  //thread t7(consumeItem1, 7); thread t8(consumeItem1, 8);

	SYSTEM_INFO sysinfo;
	GetSystemInfo(&sysinfo);
	int num_cpus = sysinfo.dwNumberOfProcessors;
	printf("there is %d cpus\n", num_cpus);
	DWORD_PTR affinity_mask = 1;
	if (num_cpus > 1)
	{
		affinity_mask = 1 << 0;
	}
	//BOOL rc = SetThreadAffinityMask(t.native_handle(), 0x3);//0000_0011
	//if (rc == 0) {
	//	std::cerr << "Error setting CPU affinity: " << GetLastError() << std::endl;
	//}
	//BOOL rc = SetThreadAffinityMask(t0.native_handle(), 0X10);//0001_0000
	//if (rc == 0) {
	//	std::cerr << "Error setting CPU affinity: t0" << GetLastError() << std::endl;
	//}
	//rc = SetThreadAffinityMask(t1.native_handle(), 0B00000001000000000);//0011_0000
	//if (rc == 0) {
	//	std::cerr << "Error setting CPU affinity: t1" << GetLastError() << std::endl;
	//}
	//rc = SetThreadAffinityMask(t2.native_handle(), 0B00000010000000000);//1100_0000
	//if (rc == 0) {
	//	std::cerr << "Error setting CPU affinity: t2" << GetLastError() << std::endl;
	//}
	//rc = SetThreadAffinityMask(t3.native_handle(), 0B00000010000000000);//0000_0011 0000_0000
	//if (rc == 0) {
	//	std::cerr << "Error setting CPU affinity: t3" << GetLastError() << std::endl;
	//}
	//rc = SetThreadAffinityMask(t4.native_handle(), 0B00000100000000000);//0000_0011 0000_0000
	//if (rc == 0) {
	//	std::cerr << "Error setting CPU affinity: t5" << GetLastError() << std::endl;
	//}
	t.join();
	//printf("t.join() end\n");
	t0.join();
	...
	//t3.join();//t4.join();	//printf("t3.join()end \n"); 
	...
	write_memorypool(memorypool_used_store, count_memorypool_used);
	printf("write_memorypool success!!!\n");
	free(speed_store);
	free(MaxDRAMFillLevel);
	free(maxbufferused);
	free(memorypool_used_store);
	for(int j=0;j<4;j++)
		printf("mnn_thread[%d] %d ,mnn_thread_w[%d] %d\n", j, mnn_thread[j], j, mnn_thread_w[j]);
...
void adq_perform_transfer_test(void* adq_cu, int adq_num, int adq_type) {
	...
	do {
		...
		while (buffers_filled > 0)
		{
			if (mem_flag[memory_pool_num] == 0)
			{
				...
				alt_node[bulk_order].data_bytes = sizeof(short) * memsize;
				alt_node[bulk_order].header_number = memheadsize;
				//q.enqueue(alt_node);
			...
			//if (memory_pool_used > tgtbuf_num - 2)
			//{ 
			//	a++;
			//	//std::this_thread::yield();
			//	//std::this_thread::sleep_for(std::chrono::microseconds(50));//1000msMMP
			//		//microseconds(300));//1000DRAM2000DRMA4000DRAM8000DRAM30000DRAM300000DRAm
			//	//mmmproducer++;
			//}
		}
	...
	{
		printf("\n\nDone.\n\n");
		printf("fetch data time %d ms count_record %d\n", timer_time_ms(1), count_record);
